...
interface DistributionListProps {
  ...
  onOpenSimulation: (distribution: Distribution) => void;
}
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:opacity-60"
          >
            <Plus className="h-4 w-4 mr-2" />
            {isCreateLoading ? 'Criando...' : 'Nova simulacao'}
          </button>
          <button
            onClick={onCreate}
            disabled={isCreateLoading}
