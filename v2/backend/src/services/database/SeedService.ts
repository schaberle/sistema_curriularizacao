...
export class SeedService {
  ...
  private readonly RETRY_MAX_ATTEMPTS = 4;
  private readonly RETRY_BASE_DELAY_MS = 180;
  ...
  private isTransientNetworkError(error: any): boolean {
    ...
    const raw = `${message} ${details} ${code}`;
  ...
  private async withRetry<T>(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const delay = this.RETRY_BASE_DELAY_MS * Math.pow(2, attempt - 1) + jitter;
        console.warn(`[SeedService] retry ${attempt}/${maxAttempts - 1} for ${label} after transient error: ${error?.message || error}`);
        await this.wait(delay);
      }
    }

    const baseMessage = String(lastError?.message || lastError || 'Erro desconhecido');
    throw new Error(`[SeedService] ${label} failed after retries: ${baseMessage}`);
  }
  ...
  async seedDistribution(
    ...
    const distribution = await this.withRetry(
      'getDistribution',
      () => this.database.getDistribution(distributionId)
    );
    if (!distribution) {
    ...
    const themes = await this.withRetry(
      'getThemesByDistribution',
      () => this.database.getThemesByDistribution(distributionId)
    );
    if (themes.length === 0) {
    ...
    await this.withRetry(
      'clearDistributionGroups',
      () => this.database.clearDistributionGroups(distributionId)
    );
    await this.withRetry(
      'clearDistributionStudents',
      () => this.database.clearDistributionStudents(distributionId)
    );
    ...
    for (const { course, phase } of studentDistribution) {
      ...
      const plannedStudentId = randomUUID();
      const studentId = await this.withRetry(
        'createStudent',
        () => this.database.createStudent(name, course, phase, distributionId, plannedStudentId)
      );
      ...
      for (let i = 0; i < studentIds.length; i++) {
        ...
        for (const pref of preferences) {
          await this.withRetry(
            ...
            () => this.database.setStudentPreference(studentId, pref.themeId, pref.rank)
          );
          preferencesCreated++;
        }
      }
    ...
    if (generateAffinities) {
      ...
      for (const aff of affinities) {
        await this.withRetry(
          'addStudentAffinity',
          () => this.database.addStudentAffinity(aff.studentId, aff.targetStudentId, aff.level)
        );
        affinitiesCreated++;
      }
    }
  ...
  async seedAffinities(
    ...
    const studentIds = await this.withRetry(
      'getStudentIdsByDistribution',
      () => this.database.getStudentIdsByDistribution(distributionId)
    );
