  ...
  const checks: ValidationCheck[] = [
    // CRITICAL: Minimum students
    {
      ...
      message: stats.totalStudents >= 4
        ...
        : `Apenas ${stats.totalStudents} alunos. Necessário: mínimo 4`,
      severity: 'critical'
    },
    ...
    {
      id: 'preference-rate',
      label: 'Taxa de preferências',
      status: stats.preferenceCompletionRate >= 50 ? 'pass' : 'warning',
      message: `${stats.preferenceCompletionRate.toFixed(1)}% dos alunos registraram preferências`,
    ...
    // RECOMMENDED: Electrical engineers availability
    {
      ...
      label: 'Engenheiros Elétricos',
      status: stats.courseBreakdown.electrical >= Math.ceil(stats.totalStudents / 4)
        ? 'pass'
        : 'warning',
      message: `${stats.courseBreakdown.electrical} EE disponíveis (${Math.ceil(stats.totalStudents / 4)} recomendado)`,
      severity: 'recommended'
    }
  ];
...
export function Step3_Phase1ConfigPage() {
  ...
  const handleResetToDefaults = () => {
    ...
    addToast({
      type: 'success',
      message: 'Parâmetros resetados para o padrão'
    });
  };
  ...
  const handleNext = () => {
    if (!validation.canProceed) {
      addToast({
        type: 'error',
        message: 'Corrija os problemas críticos antes de prosseguir'
      });
      return;
    }
